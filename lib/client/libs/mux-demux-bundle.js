...
var through = require('through')
  ...
  // minimize bandwdith by sending ints instead of event names
  , eventIds = [
    'new',      // 0
    'data',     // 1
    'end',      // 2
  ...
  // create map of event names to ids to make the code easier to follow
  , eventId = {}
  eventIds.forEach(function(name, i){ eventId[name] = i })
...
function MuxDemux (opts, onConnection) {
  ...
  var md = through(function (data) {
    ...
    // translate event id to name
    data[0] = eventIds[data[0]]
  ...
  function createStream(id, meta, opts) {
    ...
    var s = through(function (data) {
      ...
      md.emit('data', [s.id, eventId['data'], data])
    }, function () {
      md.emit('data', [s.id, eventId['end'] ])
      if (this.readable && !opts.allowHalfOpen && !this.ended) {
        this.emit("end")
      }
    })
    s.pause = function () {
      md.emit('data', [s.id, eventId['pause'] ])
    }
    s.resume = function () {
      md.emit('data', [s.id, eventId['resume'] ])
    }
    s.error = function (message) {
      md.emit('data', [s.id, eventId['error'], message])
    }
    s.once('close', function () {
      delete streams[id]
      streamCount --
      md.emit('data', [s.id, eventId['close'] ])
      if(streamCount === 0)
        md.emit('zero')
    })
  ...
  outer.createStream = function (meta, opts) {
    ...
    var s = createStream(streamId++, meta, opts)
    var _opts = {writable: opts.readable, readable: opts.writable}
    md.emit('data', [s.id, eventId['new'], {meta: meta, opts: _opts}])
    return s
  }
